// GitLens — terminal-first send with agent auto-launch + browser AI send.
//
// Two send paths (both free, no API keys):
// - TERMINAL: `gitlens.sendSelection` formats the selection as markdown,
...
// - BROWSER: `gitlens.sendToBrowser` formats the same payload, copies it to
...
const KEY_REMEMBERED_TERMINAL = "gitlens.rememberedTerminalName";
// Session pin for clipboard-only (set via chooseTarget).
const KEY_FORCE_CLIPBOARD = "gitlens.forceClipboard";
...
const KEY_AGENT_MAP = "gitlens.terminalAgents";
// Last browser AI picked (e.g. "chatgpt"). Used by `defaultBrowser: "last"`.
const KEY_LAST_BROWSER = "gitlens.lastBrowserId";
...
interface AgentDef {
  ...
  label: string;
  description: string;
  // Default launch command; overridable via `gitlens.agentCommands`.
...
interface BrowserDef {
  ...
  label: string;
  description: string;
  // Default chat URL; overridable via `gitlens.browserUrls`.
  defaultUrl: string;
}
...
async function sendSelection(
  ...
  if (!editor) {
    void vscode.window.showWarningMessage("GitLens: No active editor.");
    return;
  }
  if (editor.selection.isEmpty) {
    void vscode.window.showWarningMessage(
      "GitLens: Select some code first."
    );
    return;
  }
  ...
  const config: vscode.WorkspaceConfiguration =
    vscode.workspace.getConfiguration("gitlens");
  ...
  if (forceClipboard || defaultTarget === "clipboard") {
    await copyToClipboard(
      payload,
      `$(clippy) GitLens: copied ${relativePath} (lines ${startLine}-${endLine}) — clipboard-only mode`
    );
    return;
  }
...
async function createAndLaunchFlow(
  ...
  const dest: DestinationPick | undefined = await pickDestination(
    "GitLens: no terminal open — launch a terminal agent, open a browser AI, or clipboard"
  );
  if (!dest) {
    return false;
  }
  if (dest.destKind === "clipboard") {
    await copyToClipboard(payload, "$(clippy) GitLens: copied — no terminal open");
    return false;
  }
...
async function ensureAgentAndPaste(
  ...
  const picked: EnsPick | undefined =
    await vscode.window.showQuickPick<EnsPick>(items, {
      placeHolder: `GitLens: "${target.name}" has no known AI agent — launch one or paste anyway?`,
    });
  if (!picked) {
    return;
  }
  ...
  if (picked.pickKind === "clipboard") {
    await copyToClipboard(
      payload,
      "$(clippy) GitLens: copied — terminal left untouched"
    );
    return;
  }
...
async function launchAgentAndPaste(
  ...
  try {
    ...
    vscode.window.setStatusBarMessage(
      `$(sync~spin) GitLens: starting ${agent.id} in "${terminal.name}"…`,
      delayMs
    );
    ...
    await copyToClipboard(
      payload,
      "$(clippy) GitLens: could not launch agent — copied to clipboard instead"
    );
  }
}
...
async function pasteWithOptionalPrompt(
  ...
  try {
    ...
    terminal.show();
    vscode.window.setStatusBarMessage(
      `$(terminal) GitLens: pasted ${relativePath} (lines ${startLine}-${endLine}) ${agentTag}in "${terminal.name}" — review & press Enter`,
      3000
    );
  } catch {
    await copyToClipboard(
      payload,
      "$(clippy) GitLens: terminal paste failed — copied to clipboard instead"
    );
  }
}
...
async function withOptionalInstruction(
  ...
  const instruction: string | undefined =
    await vscode.window.showInputBox({
      ...
      prompt: `GitLens: what should ${agentTag || "the AI"}do with ${relativePath} (lines ${startLine}-${endLine})?`,
    });
...
// ---------------------------------------------------------------------------
// Browser command: `gitlens.sendToBrowser` — copy payload, open the chat
// site, user pastes once. Honors `gitlens.defaultBrowser` ("ask" | "last"
...
async function sendToBrowser(
  ...
  if (browser === "clipboard") {
    await copyToClipboard(
      payload,
      `$(clippy) GitLens: copied ${relativePath} (lines ${startLine}-${endLine})`
    );
    return;
  }
  await sendToBrowserTarget(
...
async function resolveBrowser(
  ...
  const picked: BrowserPick | undefined =
    await vscode.window.showQuickPick<BrowserPick>(items, {
      placeHolder: "GitLens: pick a browser AI (code is copied — paste once with Ctrl+V)",
    });
  if (!picked) {
    return undefined;
  }
  ...
  try {
    ...
    if (opened) {
      void vscode.window.showInformationMessage(
        `GitLens: ${relativePath} (lines ${startLine}-${endLine}) copied — paste once (Ctrl+V) in ${browser.id}.`,
        "Copy again"
      ...
      vscode.window.setStatusBarMessage(
        `$(globe) GitLens: opened ${browser.id} — code is in your clipboard, press Ctrl+V there`,
        5000
      );
    } else {
      vscode.window.setStatusBarMessage(
        "$(clippy) GitLens: browser would not open — code copied to clipboard instead",
        3000
      );
    }
  } catch {
    vscode.window.setStatusBarMessage(
      "$(clippy) GitLens: could not open browser — code copied to clipboard instead",
      3000
    );
  }
}
...
async function resolveTargetTerminal(
  ...
  const pickedName: string | undefined =
    await vscode.window.showQuickPick(
      ...
      {
        placeHolder:
          "GitLens: pick the terminal (Claude Code, Copilot CLI, Codex, OpenCode, aider, …)",
      }
    );
  if (!pickedName) {
    return undefined;
...
async function chooseTarget(
    ...
    await vscode.window.showQuickPick<TargetPick>(items, {
      placeHolder:
        openTerminals.length === 0
          ? "GitLens: no terminals open — pick clipboard or auto"
          : "GitLens: choose send target",
    });
  if (!picked) {
    return;
  }
  ...
  if (picked.targetKind === "auto") {
    ...
    await context.workspaceState.update(KEY_FORCE_CLIPBOARD, false);
    vscode.window.setStatusBarMessage(
      `$(terminal) GitLens: will send to "${picked.terminalName}"`,
      2000
    );
  }
}
